  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Stack(
        ...
        children: [
          model.kitchenMediaList.isNotEmpty
              ? AspectRatio(
                  aspectRatio: aspectRatio ?? 1225 / 250,
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(24),
                    child: model.kitchenMediaList.first.mediaType ==
                            MediaType.image
                        ? Image.file(
                            File(model.kitchenMediaList.first.path),
                            fit: fitType ?? BoxFit.fill,
                            width: imageWidth,
          ...
          enableShadow
              ? CustomInnerShadowInLastKitchen(
                  aspectRatio: aspectRatioInner,
                  desc: model.kitchenDesc ?? "",
                  name: model.kitchenName ?? "",
                )
              : const SizedBox(),
        ],
      ),
